...
// NOTE: "Voting Taluka" now reads from `votingTaluka` (the real Voting
// Taluka column from the source Excel, col index 7), not `taluka`
// (which holds the Institute Taluka, col index 5). See importVoters.js
// and models/Voter.js for the corresponding backend fix.
const EXPORT_COLUMNS = [
  ...
  { header: "Voting Taluka", key: "votingTaluka" },
  { header: "Age", key: "age" },
  { header: "Gen", key: "gen" },
];

// Columns the results table header allows the user to sort by: Voter
// Name (existing) and Institute Name (new). Village / Voting Taluka /
// Age / Gen stay non-sortable per request.
const SORTABLE_COLUMNS = {
  electorName: { getValue: (v) => cleanVoterName(v.electorName) },
  institute: { getValue: (v) => v.institute },
};
...
export default function SearchResults({
  ...
  // Generic column sort: { key: 'electorName' | 'institute' | null,
  // direction: 'asc' | 'desc' | null }. Only one column is sorted at a
  // time - clicking a header cycles asc -> desc -> off for that column,
  // and switches the active column if a different header is clicked.
  ...
  // Alphabetical sort on whichever column is active, applied on top of
  // the name filter. Falls back to the filtered (unsorted) list when no
  // column is currently sorted.
  const displayedVoters = useMemo(() => {
    const { key, direction } = sortConfig;
    if (!key || !direction) return filteredVoters;

    const { getValue } = SORTABLE_COLUMNS[key];
  ...
  // Cycles the given column through asc -> desc -> off. Clicking a
  // different column than the one currently active starts that column
  // fresh at asc, regardless of the previous column's state.
        ...
        <table className="results-table">
          <thead>
            <tr>
              ...
              <th className="col-name" {...sortableHeaderProps("electorName")}>
                <span
                  style={{
                    display: "inline-flex",
                    alignItems: "center",
              ...
              <th className="col-narrow">Voting Taluka</th>
              <th className="col-narrow">Age</th>
              <th className="col-narrow">Gen</th>
              ...
              displayedVoters.map((v, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="voter-name col-name">
                      {cleanVoterName(v.electorName)}
                    </td>
                    ...
                    <td className="col-narrow">{v.votingTaluka || "-"}</td>
                    <td className="col-narrow">{v.age || "-"}</td>
                    <td className="col-narrow">{v.gen || "-"}</td>
