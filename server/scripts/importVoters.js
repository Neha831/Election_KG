...
//   const dataRows = rows.slice(1); // row 0 is the header (District, Part, Sr No, ...)
...
// server/scripts/importVoters.js
//
// Usage:
//   node scripts/importVoters.js                 (uses ./data/Link_Data_File.xlsx)
//   node scripts/importVoters.js /path/to/file.xlsx
//
// Requires: npm install xlsx dotenv   (mongoose you already have)
...
async function main() {
  ...
  // District, Part, Sr No, Elector Name, Mobile Number, Address, Institute, Village, Taluka
  const docs = dataRows.map((r) => ({
    district: clean(r[0]),
    part: clean(r[1]),
    srNo: clean(r[2]),
